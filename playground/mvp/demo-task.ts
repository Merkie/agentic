...
const agentic = createAgentic({
	...
	onEvent: (e) => {
		...
		else if (e.type === "retry") console.log(`  [retry ${e.attempt}] in ${e.delayMs}ms: ${e.error.message}`);
		else if (e.type === "run-end") console.log(`  [run-end] ${e.status} cost=$${e.totals.cost.toFixed(6)}`);
	},
});
